...
const getFontObjectValue = (inp, clean = true, keys = ['en', 'zh', 'zh-TW', 'zh-tw']) => {
...
const rFontWeights = /ExtraLight|Light|Medium|Bold|Heavy|Black/i;
...
class FontSubseter {
    ...
    loadInfo(buffer, lang = 'en') {
        ...
        const getName = (key = 'fontFamily', defaultVal = '') => {
            return getFontObjectValue(names[key], true, readKeys) || defaultVal || '';
        };
        ...
        const fontSubfamily = fontWeightsMap[weight] || getName('fontSubfamily', 'Regular');

        let fullName = getName('fullName', fontFamily);
        if(fullName.indexOf(fontSubfamily) < 0) {
