    ...
    makeThumbnail(buffer, text = '', {
        ...
        const paths =
            chars.map(char => {
                ...
                // 字形不存在且有默认字体时使用默认字体回退
                if(path.commands.length === 0 && defaultFont) {
                    return defaultFont.getPath(char, x, y, fontSize);
                }
                return path;
            })
        ...
        const pathDatas = paths.map((path, i) => {
            const bbox = path.getBoundingBox();
            const offsets = [
                Math.floor(bbox.x1 - bboxWidth),
                offsetY
            ];
            ...
            // 计算新的 bbox 宽度，每个字符间隔 5 px
            bboxWidth += bbox.x2 - x + 5;
            return pathData;
        });
